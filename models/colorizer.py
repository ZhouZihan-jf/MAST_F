...
class Colorizer(nn.Module):
    ...
    def prep(self, image, HW):
        ...
        x = image.float()[:, :, ::self.D, ::self.D]  # ::4,相当于采样，256*256->64*64
    ...
    def forward(self, feats_r, feats_t, quantized_r, ref_index, current_ind, dil_int=15):
        ...
        # ref_index中距离带预测帧超过15的个数(最多为2，因为只有0，5帧有可能)
        ...
        for searching_index in range(nsearch):
            ...
            grid = torch.stack([_x, _y], dim=-1).unsqueeze(-2).unsqueeze(-2) \
                .reshape(1, self.memory_patch_P, self.memory_patch_P, 1, 1, 2).contiguous().float().to(
        ...
        # 前一张图片drop掉的通道（训练） / ref帧的mask（测试）们
